...
public class PointerView extends View {
	...
	public PointerView(Context context, AttributeSet attrs) {
		...
		// Log.d(TAG, "Comparing default velocity tracker strategy with " +
		// altStrategy);
	...
	public static class PointerState {
		...
		// Position estimator.
		// private VelocityTracker.Estimator mEstimator = new
		// VelocityTracker.Estimator();
		// private VelocityTracker.Estimator mAltEstimator = new
		// VelocityTracker.Estimator();
	...
	private void drawOval(Canvas canvas, float x, float y, float major,
			float minor, float angle, Paint paint) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int p = 0; p < NP; p++) {
			...
			if (drawn) {
				...
				// float lx = ps.mEstimator.estimateX(-ESTIMATE_PAST_POINTS *
				// ESTIMATE_INTERVAL);
				// float ly = ps.mEstimator.estimateY(-ESTIMATE_PAST_POINTS *
				// ESTIMATE_INTERVAL);
				// for (int i = -ESTIMATE_PAST_POINTS + 1; i <=
				// ESTIMATE_FUTURE_POINTS; i++) {
				...
				if (mAltVelocity != null) {
					...
					// lx = ps.mAltEstimator.estimateX(-ESTIMATE_PAST_POINTS *
					// ESTIMATE_INTERVAL);
					// ly = ps.mAltEstimator.estimateY(-ESTIMATE_PAST_POINTS *
					// ESTIMATE_INTERVAL);
					// for (int i = -ESTIMATE_PAST_POINTS + 1; i <=
					// ESTIMATE_FUTURE_POINTS; i++) {
					// float x = ps.mAltEstimator.estimateX(i *
					// ESTIMATE_INTERVAL);
					// float y = ps.mAltEstimator.estimateY(i *
					// ESTIMATE_INTERVAL);
					// canvas.drawLine(lx, ly, x, y, mPaint);
					// lx = x;
					// ly = y;
			...
			if (mCurDown && ps.mCurDown) {
				// Draw crosshairs.
				canvas.drawLine(0, ps.mCoords.y, getWidth(), ps.mCoords.y,
						mTargetPaint);
				canvas.drawLine(ps.mCoords.x, 0, ps.mCoords.x, getHeight(),
						mTargetPaint);
				...
				drawOval(canvas, ps.mCoords.x, ps.mCoords.y,
						ps.mCoords.touchMajor, ps.mCoords.touchMinor,
						ps.mCoords.orientation, mPaint);

				// Draw current tool ellipse.
				mPaint.setARGB(255, pressureLevel, 128, 255 - pressureLevel);
				drawOval(canvas, ps.mCoords.x, ps.mCoords.y,
						ps.mCoords.toolMajor, ps.mCoords.toolMinor,
						ps.mCoords.orientation, mPaint);
				...
				float orientationVectorX = (float) (Math
						.sin(ps.mCoords.orientation) * arrowSize);
				float orientationVectorY = (float) (-Math
						.cos(ps.mCoords.orientation) * arrowSize);
				if (ps.mToolType == MotionEvent.TOOL_TYPE_STYLUS
						|| ps.mToolType == MotionEvent.TOOL_TYPE_ERASER) {
					// Show full circle orientation.
					canvas.drawLine(ps.mCoords.x, ps.mCoords.y, ps.mCoords.x
							+ orientationVectorX, ps.mCoords.y
							+ orientationVectorY, mPaint);
				} else {
					// Show half circle orientation.
					canvas.drawLine(ps.mCoords.x - orientationVectorX,
							ps.mCoords.y - orientationVectorY, ps.mCoords.x
									+ orientationVectorX, ps.mCoords.y
									+ orientationVectorY, mPaint);
				}

				// Draw the tilt point along the orientation arrow.
				float tiltScale = (float) Math.sin(ps.mCoords
						.getAxisValue(MotionEvent.AXIS_TILT));
				canvas.drawCircle(
						ps.mCoords.x + orientationVectorX * tiltScale,
						ps.mCoords.y + orientationVectorY * tiltScale, 3.0f,
						mPaint);
			}
		}
	}
	...
	public void addPointerEvent(MotionEvent event) {
		...
		if (action == MotionEvent.ACTION_DOWN
			...
			final int index = (action & MotionEvent.ACTION_POINTER_INDEX_MASK) >> MotionEvent.ACTION_POINTER_INDEX_SHIFT; // will
																															// be
																															// 0
																															// for
																															// down
			// Clear the data when another motion event begin
			if (action == MotionEvent.ACTION_DOWN) {
				for (int p = 0; p < NP; p++) {
					final PointerState ps = mPointers.get(p);
					ps.clearTrace();
